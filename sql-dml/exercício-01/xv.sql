...
SELECT a.nome, a.sobrenome, g.ano, count(*)  
FROM aluno a
    INNER JOIN aluno_grade ag ON (ag.idAluno = a.idAluno)
    INNER JOIN grade g ON (g.idGrade = ag.idGrade)
GROUP BY a.idAluno, g.ano
    -- Atualmente não existem alunos que tenham duas matriculas para o mesmo curso e mesma grade.ano
